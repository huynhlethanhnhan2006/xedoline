...
void corner_steering(int steering_time)
{
  time = millis();
  if (S1 == 1)
  {
...
void control()
{
  ...
  // onBuzzer();
  time = millis();
  while (millis() - time < 1700 && state == 3)
  {
    // if(millis() - time >= 250) offBuzzer(); 
    readlightsensor();
    line_tracking();
  ...
  // onBuzzer();
  time = millis();
  while (millis() - time < 8000 && state == 4)
  {
    readlightsensor();
    line_tracking();
  }
  while (state == 4)
  {
    readlightsensor();
    line_tracking();
    if (S1 == 0 && S2 == 0 && S3 == 0 && S4 == 0 && state == 4)
    {
    // go_back(255, 255);
    // time = millis();
    // while (millis() - time < 100)
    // {}
      ...
      time = millis();
      while ((S2 == 0 && S3 == 0) || millis() - time < 320)
      {
        strong_right_steering(240, 240);
        readlightsensor();
      }
      stop();
      break;
